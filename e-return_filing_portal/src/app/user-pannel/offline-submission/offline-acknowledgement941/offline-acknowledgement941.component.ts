...
@Component({
  selector: 'app-offline-acknowledgement941',
  templateUrl: './offline-acknowledgement941.component.html',
  styleUrls: ['./offline-acknowledgement941.component.css']
})
export class OfflineAcknowledgement941Component implements OnInit {
  userTin: any;
  checkIsLoggedIn: any;
  ...
  initializeWealthSurchargePaid(event) {
    this.WealthSurchargePaid = this.commaSeparator.currencySeparatorBD(this.commaSeparator.removeComma(event.target.value, 0));
  }

  initializeAmountPaidAdjusted(event) {
