...
@Component({
  selector: 'app-authentication-configuration',
  templateUrl: './authentication-configuration.component.html',
  styleUrls: ['./authentication-configuration.component.scss']
})
export class AuthenticationConfigurationComponent implements OnInit {

  autheticationConfigForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.autheticationConfigForm = this.fb.group({

      Auth_Config: this.fb.array([this.fb.group({
        id : new FormControl(),
  ...
  {
    let getData: any;
    this.apiService.get(environment.management_base_url + '/ereturnmanagement/v2/api/get-module-wise-config-info/'+'AUTHENTICATION')
      .subscribe(result => {
  ...
  get table_1_List() {
    return this.autheticationConfigForm.get('Auth_Config') as FormArray;
  }
