...
@Component({
  selector: 'app-ledger-configuration',
  templateUrl: './ledger-configuration.component.html',
  styleUrls: ['./ledger-configuration.component.scss']
})
export class LedgerConfigurationComponent implements OnInit {

  ledgerConfigForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.ledgerConfigForm = this.fb.group({

      Ledger_Config: this.fb.array([this.fb.group({
        id : new FormControl(),
  ...
  {
    let getData: any;
    this.apiService.get(environment.management_base_url + '/ereturnmanagement/v2/api/get-module-wise-config-info/'+'LEDGER')
      .subscribe(result => {
  ...
  get table_1_List() {
    return this.ledgerConfigForm.get('Ledger_Config') as FormArray;
  }
