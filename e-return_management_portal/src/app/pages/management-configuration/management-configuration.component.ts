...
@Component({
  ...
  styleUrls: ['./management-configuration.component.scss']
})
export class ManagementConfigurationComponent implements OnInit {

  managementConfigForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.managementConfigForm = this.fb.group({

      Management_Config: this.fb.array([this.fb.group({
        id : new FormControl(),
  ...
  {
    let getData: any;
    this.apiService.get(environment.management_base_url + '/ereturnmanagement/v2/api/get-module-wise-config-info/'+'MANAGEMENT')
      .subscribe(result => {
  ...
  get table_1_List() {
    return this.managementConfigForm.get('Management_Config') as FormArray;
  }
     ...
     let management_config = [];
       ...
       this.table_1_List.controls.forEach((element, index) => {
        ...
        let validTo = element.value.valid_to ? moment(element.value.valid_to, 'DD-MM-YYYY') : '';
         ...
         management_config.push(obj);
       });


       if(this.validateErrorIndexes())
       {
        this.apiService.post(environment.management_base_url + '/ereturnmanagement/v2/api/save-config-info-list', management_config)
        .subscribe(result => {
          if (result != null) {
       ...
       console.log("json result ",management_config);
