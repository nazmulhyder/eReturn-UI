...
@Component({
  ...
  styleUrls: ['./basic-configuration.component.scss']
})
export class BasicConfigurationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.basicConfigForm = this.fb.group({
      Basic_Config: this.fb.array([this.fb.group({
        id : new FormControl(),
  ...
  {
    let getData: any;
    this.apiService.get(environment.management_base_url + '/ereturnmanagement/v2/api/get-module-wise-config-info/'+'BASIC')
      .subscribe(result => {
  ...
  get table_1_List() {
    return this.basicConfigForm.get('Basic_Config') as FormArray;
  }
  ...
  saveData()
  {
     let filing_config = [];
       ...
       console.log('post data', filing_config);
       if(this.validateErrorIndexes())
       {
