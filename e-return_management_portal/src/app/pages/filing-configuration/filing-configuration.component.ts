...
@Component({
  selector: 'app-filing-configuration',
  templateUrl: './filing-configuration.component.html',
  styleUrls: ['./filing-configuration.component.scss']
})
export class FilingConfigurationComponent implements OnInit {

  filingConfigForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.filingConfigForm = this.fb.group({

      Filing_Config: this.fb.array([this.fb.group({
        id : new FormControl(),
        incomeHeadType : new FormControl(),
  ...
  {
    let getData: any;
    this.apiService.get(environment.management_base_url + '/ereturnmanagement/v2/api/get-module-wise-config-info/'+'FILING')
      .subscribe(result => {
            ...
            if(getData.length != 0)
            {
              ...
              getData.forEach((element,index) => {    
                this.table_1_List.push(this.fb.group({
                  ...
                  incomeHeadType: new FormControl(element.moduleType),
            ...
            console.log('get data', this.table_1_List.controls);

      })
  }

  get table_1_List() {
    return this.filingConfigForm.get('Filing_Config') as FormArray;
  }
  ...
  {
     let filing_config = [];
       console.log('save total data', this.table_1_List.controls)
       this.table_1_List.controls.forEach((element, index) => {
        console.log('valid to date', element.value.valid_to);
        ...
        let validFrom = new Date(element.value.valid_from);
        let validTo = new Date(element.value.valid_to);
        let obj = {
          //  "id": element.value.id? element.value.id :0,
          "activeStatus": true,
          "afterSubmissionLockStatus": true,
          "moduleType": element.value.incomeHeadType ? element.value.incomeHeadType : '',
         ...
         filing_config.push(obj);
       });


       if(this.validateErrorIndexes())
       {
        this.apiService.post(environment.management_base_url + '/ereturnmanagement/v2/api/save-config-info-list', filing_config)
        .subscribe(result => {
          if (result != null) {
       ...
       console.log("json result ",filing_config);
  ...
  {
    this.filing_showError[i].incomeType_showError = false;
  ...
  validateErrorIndexes() : boolean
  {
      ...
      if(element.value.incomeHeadType == null || element.value.incomeHeadType == '' ||  element.value.incomeHeadType == '0')
      {
          this.filing_showError[index].incomeType_showError = true;
  ...
  {
    if(formControlName == 'incomeHeadType')
    this.filing_showError[i].incomeType_showError = false;
